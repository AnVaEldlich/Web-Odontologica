...
document.addEventListener('DOMContentLoaded', validateRegisterForm);
        ...
        // Obtiene el formulario y el mensaje de confirmación por su ID
        const form = document.getElementById('odontologo-form');
        const confirmationMessage = document.getElementById('confirmation-message');

        // Escucha el evento de envío del formulario
        form.addEventListener('submit', function(event) {
            // Previene el comportamiento por defecto de envío del formulario (recargar la página)
            event.preventDefault();
            
            // Muestra el mensaje de confirmación
            confirmationMessage.classList.remove('hidden');

            // Oculta el formulario después de 2 segundos para simular el envío
            setTimeout(() => {
                form.reset(); // Limpia el formulario
                confirmationMessage.classList.add('hidden'); // Vuelve a ocultar el mensaje
